...
const dfsWithBranchSpecificVisited = async (filePath, depthLimit, branchVisitedMap, promptsByDepth, allDirectories, options) => {
  const stack = [{ path: filePath, depth: 0, context: Config.systemPrompt, prompt: "" }];
  ...
  while (stack.length > 0) {
    ...
    const { hasDirective, prompt: promptText, directive, fileContent } = await checkDirective(currentPath);
    ...
    if (depth < depthLimit) {
      const deps = precinct.paperwork(fileURLToPath(currentPath), {
        includeCore: false,
        fileSystem: fs,
      });
      ...
      for (let dep of deps) {
        ...
        try{
          ...
          resolvedPath = path.resolve(path.dirname(fileURLToPath(currentPath)), dep)
        }

        const {status, fullPath} = checkFileExists(resolvedPath, Config.extensions)

        if (status === true) {
          stack.push({ 
            path: pathToFileURL(fullPath), 
            depth: depth + 1, 
...
const iterativeDeepeningDFS = async (filePath, maxDepth, allDirectories, options) => {
  let arrOfArrs = [];

  for (let depth = 0; depth <= maxDepth; depth++) {
    const branchVisitedMap = new Map(); // Map to hold visited sets for each depth
    const promptsByDepth = [];
    await dfsWithBranchSpecificVisited(filePath, depth, branchVisitedMap, promptsByDepth, allDirectories, options);
